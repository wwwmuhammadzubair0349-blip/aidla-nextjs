...
export default function UserShop() {
  ...
  const [view, setView]             = useState("shop"); // shop | product | cart | orders | checkout
  ...
  const [checkoutItem, setCheckoutItem] = useState(null); // { product, variant, quantity }
  ...
  const cartTotal = useMemo(() => {
    const feePct = (data.fees || []).reduce((s, f) => s + Number(f.percentage), 0);
    return (data.cart || []).reduce((s, c) => {
      const price = discountedPrice(c.price_coins, c.discount_pct);
      const variantExtra = Number(c.variant_extra || 0);
      const subtotal = (price + variantExtra) * c.quantity;
  ...
  const handleUpdateCart = async (productId, variantId, quantity) => {
    await supabase.rpc("shop_update_cart", { p_product_id: productId, p_variant_id: variantId || null, p_quantity: quantity });
    await loadAll();
  };
  ...
  const handlePlaceOrder = async () => {
    ...
    showMsg(`✅ Order placed! Ref: ${d.txn_no}${d.cashback > 0 ? ` · +${fmt(d.cashback)} coins cashback!` : ""}`);
          ...
          <>
            ...
            {filteredProducts.some(p => p.is_featured) && (
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={S.grid}>
                  {filteredProducts.filter(p => p.is_featured).map(p => <ProductCard key={p.id} p={p} onView={() => { setSelectedProduct(p); setSelectedVariant(null); setView("product"); }} onWishlist={() => handleToggleWishlist(p.id)} onCart={() => handleUpdateCart(p.id, null, 1)} onBuy={() => handleBuyNow(p)} inWishlist={p.in_wishlist} balance={balance} />)}
                </div>
              </div>
            )}
            ...
            {filteredProducts.length === 0 ? (
              ...
              <div style={S.grid}>
                {filteredProducts.map(p => <ProductCard key={p.id} p={p} onView={() => { setSelectedProduct(p); setSelectedVariant(null); setView("product"); }} onWishlist={() => handleToggleWishlist(p.id)} onCart={() => handleUpdateCart(p.id, null, 1)} onBuy={() => handleBuyNow(p)} inWishlist={p.in_wishlist} balance={balance} />)}
              </div>
            )}
          </>
        )}
        ...
        {view === "product" && selectedProduct && (
          <div>
            ...
            <div style={S.card}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10 }}>
                ...
                <div style={{ textAlign: "right" }}>
                  {selectedProduct.discount_pct > 0 && (
                    <div style={{ fontSize: 12, color: "#94a3b8", textDecoration: "line-through" }}>{fmt(selectedProduct.price_coins)} coins</div>
                  )}
                  <div style={{ fontSize: 24, fontWeight: 900, color: "#1e3a8a" }}>
                    {fmt(selectedProduct.price_coins * (1 - (selectedProduct.discount_pct || 0) / 100))} coins
                  </div>
                  ...
                  {selectedProduct.cashback_pct > 0 && <div style={{ fontSize: 12, color: "#7c3aed", marginTop: 4 }}>🎁 {selectedProduct.cashback_pct}% cashback</div>}
                  {selectedProduct.sale_ends_at && new Date(selectedProduct.sale_ends_at) > new Date() && (
                    <div style={{ fontSize: 11, color: "#ef4444", marginTop: 4 }}>⏰ Sale ends {new Date(selectedProduct.sale_ends_at).toLocaleDateString()}</div>
                  )}
                </div>
              </div>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                ...
                <button style={{ ...S.btn, background: "linear-gradient(135deg,#475569,#64748b)", flex: 1 }} onClick={() => { handleUpdateCart(selectedProduct.id, selectedVariant?.id, 1); showMsg("Added to cart!"); }}>🛒 Add to Cart</button>
            ...
            <div style={{ ...S.card, marginTop: 12 }}>
              <div style={{ fontWeight: 800, marginBottom: 12 }}>Reviews</div>
              {(selectedProduct.review_count || 0) === 0 ? (
                <div style={{ color: "#94a3b8", fontSize: 14 }}>No reviews yet.</div>
              ) : (
                <div style={{ color: "#64748b", fontSize: 14 }}>⭐ {selectedProduct.avg_rating} average from {selectedProduct.review_count} reviews.</div>
              )}
            </div>
            ...
            <div style={{ marginTop: 16 }}>
              ...
              <div style={S.grid}>
                {(data.products || []).filter(p => p.id !== selectedProduct.id && p.category_id === selectedProduct.category_id).slice(0, 4).map(p => (
                  <ProductCard key={p.id} p={p} onView={() => { setSelectedProduct(p); setSelectedVariant(null); setView("product"); window.scrollTo(0, 0); }} onWishlist={() => handleToggleWishlist(p.id)} onCart={() => handleUpdateCart(p.id, null, 1)} onBuy={() => handleBuyNow(p)} inWishlist={p.in_wishlist} balance={balance} />
                ))}
              </div>
            </div>
          </div>
        ...
        {view === "cart" && (
          <div>
            ...
            <div style={S.card}>
              ...
              {(data.cart || []).length === 0 ? (
                <div style={{ textAlign: "center", padding: 32, color: "#94a3b8" }}>Your cart is empty. <button style={{ ...S.btn, display: "inline-block", padding: "6px 14px", fontSize: 13, marginLeft: 10 }} onClick={() => setView("shop")}>Shop Now</button></div>
              ) : (
                <>
                  {(data.cart || []).map(item => (
                    <div key={item.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: "1px solid #f1f5f9", flexWrap: "wrap", gap: 10 }}>
                      ...
                      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                        <button style={S.qtyBtn} onClick={() => handleUpdateCart(item.product_id, item.variant_id, Math.max(0, item.quantity - 1))}>−</button>
                  ...
                  <div style={{ marginTop: 14, padding: "12px 0", borderTop: "2px solid #e2e8f0" }}>
                    ...
                    <div style={{ fontSize: 12, color: "#64748b", marginTop: 4 }}>Includes fees · Final amount calculated at checkout</div>
                  </div>
                  <div style={{ fontSize: 13, color: "#64748b", marginBottom: 10 }}>To checkout, go to each product and use Buy Now.</div>
                </>
              )}
            </div>
          </div>
            ...
            <div style={S.card}>
              ...
              <div style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 10, padding: 14, marginBottom: 16 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, marginBottom: 4 }}>
                  <span>{checkoutItem.product.name} {checkoutItem.variant ? `(${checkoutItem.variant.value})` : ""}</span>
                ...
                {(data.fees || []).length > 0 && (
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "#64748b" }}>
                    <span>Fees ({(data.fees || []).reduce((s, f) => s + Number(f.percentage), 0)}%)</span>
                    <span>+{fmt(calcOrderTotal(checkoutItem.product, checkoutItem.variant, 1) - discountedPrice(checkoutItem.product.price_coins, checkoutItem.product.discount_pct) * (1 - couponDiscount / 100))} coins</span>
                  </div>
                )}
                <div style={{ borderTop: "1px solid #e2e8f0", marginTop: 10, paddingTop: 10, display: "flex", justifyContent: "space-between", fontWeight: 900, fontSize: 16 }}>
                  <span>Total</span>
                  <span style={{ color: "#1e3a8a" }}>{fmt(calcOrderTotal(checkoutItem.product, checkoutItem.variant, 1))} coins</span>
                </div>
                {checkoutItem.product.cashback_pct > 0 && (
                  <div style={{ fontSize: 12, color: "#7c3aed", marginTop: 6 }}>🎁 You'll earn {fmt(calcOrderTotal(checkoutItem.product, checkoutItem.variant, 1) * checkoutItem.product.cashback_pct / 100)} coins cashback!</div>
                )}
              </div>
              ...
              <button style={{ ...S.btn, marginTop: 14, width: "100%", opacity: submitting ? 0.6 : 1 }}
                ...
                {submitting ? "Placing Order…" : `✅ Confirm Order · ${fmt(calcOrderTotal(checkoutItem.product, checkoutItem.variant, 1))} coins`}
              </button>
            </div>
          </div>
        )}
        ...
        {view === "orders" && (
          <div>
            ...
            <div style={S.card}>
              ...
              {orders.length === 0 ? (
                <div style={{ textAlign: "center", padding: 32, color: "#94a3b8" }}>No orders yet. <button style={{ ...S.btn, display: "inline-block", padding: "6px 14px", fontSize: 13, marginLeft: 10 }} onClick={() => setView("shop")}>Shop Now</button></div>
              ) : orders.map(o => (
                <div key={o.id} style={{ border: "1px solid #e2e8f0", borderRadius: 10, padding: 14, marginBottom: 10, display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
                  <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                    ...
                    <div>
                      ...
                      {o.cashback_coins > 0 && <div style={{ fontSize: 12, color: "#7c3aed" }}>🎁 +{fmt(o.cashback_coins)} cashback</div>}
...
function ProductCard({ p, onView, onWishlist, onCart, onBuy, inWishlist, balance }) {
  ...
  return (
    <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, overflow: "hidden", cursor: "pointer", position: "relative" }}>
      ...
      <button style={{ position: "absolute", top: 8, right: 8, background: "rgba(255,255,255,0.9)", border: "none", borderRadius: "50%", width: 28, height: 28, fontSize: 16, cursor: "pointer", zIndex: 1, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={e => { e.stopPropagation(); onWishlist(); }}>
        {inWishlist ? "💛" : "🤍"}
      </button>
      <div onClick={onView}>
        {p.images?.[0] ? (
...
const S = {
  ...
  modal:   { background: "#fff", borderRadius: 16, padding: 24, width: "min(360px,100%)", boxShadow: "0 20px 60px rgba(15,23,42,0.2)" },
};
