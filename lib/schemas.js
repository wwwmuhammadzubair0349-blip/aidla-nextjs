...
export function buildFounderSchema() {
  return {
    ...
    sameAs: [
      ...
      "https://www.youtube.com/@engrmuhammadzubairafridi",
      "https://www.tiktok.com/@aidla.online",
      "https://sites.google.com/view/muhammadzubairafridi/home",
      "https://www.crunchbase.com/person/muhammad-zubair-afridi",
    ],
  };
}
...
export function buildOrganizationSchema() {
  return {
    ...
    alternateName: [
      ...
      "AIDLA - Artificial Intelligence Digital Learning Academy",
      "AIDLA Online",
      "AIDLA",
    ...
    disambiguatingDescription: SITE.businessDescription,
    slogan:      SITE.tagline,
    ...
    knowsAbout: [
      "AI Learning", "Online Education", "Global Education",
      "Daily Quizzes", "Career Tools", "CV Builder",
      "Rewards Based Learning", "EdTech", "Professional Development",
    ],
    hasOfferCatalog: {
      "@type": "OfferCatalog",
      ...
      itemListElement: [
        {
          ...
          itemOffered: {
            ...
            description: "Daily quiz competitions for global learners with AIDLA Coins rewards.",
    ...
    sameAs: [
      ...
      "https://www.linkedin.com/company/aidla-online/",
      "https://www.tiktok.com/@aidla.online",
      "https://www.youtube.com/@aidla.online",
      "https://www.crunchbase.com/organization/aidla",
      "https://www.trustpilot.com/review/aidla.online",
      "https://maps.app.goo.gl/Sgc5ieoWgwcwwXWk7",
    ],
  };
}
...
export function buildSyedSolarSchema() {
  return {
    ...
    image: SYED_SOLAR.image,
    priceRange: SYED_SOLAR.priceRange,
    address: {
      "@type":         "PostalAddress",
...
export function buildWebSiteSchema() {
  return {
    ...
    potentialAction: {
      ...
      target: {
        "@type":     "EntryPoint",
        urlTemplate:`${SITE.url}/courses?q={search_term_string}`,
      },
...
/* ══════════════════════════════════════════════════════════════
   6. WEBPAGE — per page, pass page-specific data
   (Numbers 7+ shifted up after EducationalOrganization was merged into Organization)
══════════════════════════════════════════════════════════════ */
...
export function buildFAQSchema(faqs = [], path = "") {
  const id = path ? `${SITE.url}${path}#faqpage` : `${SITE.url}/#faqpage`;
  return {
    "@type": "FAQPage",
    "@id":   id,
...
/* ══════════════════════════════════════════════════════════════
   10. HOW-TO
══════════════════════════════════════════════════════════════ */
export function buildHowToSchema(steps = [], path = "") {
  const id = path ? `${SITE.url}${path}#howto` : `${SITE.url}/#howto`;
  return {
    "@type":      "HowTo",
    "@id":        id,
    name:         "How AIDLA Works",
    description:  "Learn how to study, use AI tools, build career skills, earn AIDLA Coins, and redeem rewards on AIDLA's AI learning platform.",
...
export function buildArticleSchema({
  ...
  const founderNames = [FOUNDER.fullName, "AIDLA", "AIDLA Team", undefined, null, ""];
  const author = founderNames.includes(authorName)
    ? { "@id": `${SITE.url}/#founder` }
    : { "@type": "Person", name: authorName };
  return {
    "@type":       type,
    "@id":         `${SITE.url}/${basePath}/${slug}#article`,
    headline:      title,
    description,
    ...
    author,
    publisher:     { "@id": `${SITE.url}/#organization` },
    mainEntityOfPage: `${SITE.url}/${basePath}/${slug}`,
    inLanguage:    "en",
    ...(keywords && { keywords }),
    ...(speakableSelectors.length > 0 && {
